...
fn bench_email_length_impact(c: &mut Criterion) {
    ...
    for (name, email) in [
        ...
        group.bench_with_input(BenchmarkId::new("our_validator", name), email, |b, email| {
            b.iter(|| black_box(our_validate_email(black_box(email))))
        });
        
        group.bench_with_input(BenchmarkId::new("validator_crate", name), email, |b, email| {
            b.iter(|| black_box(validator_crate_validate(black_box(email))))
        });
    }
